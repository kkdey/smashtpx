...
smash.tpxSelect <- function(X, K, bf, initheta, alpha, tol, kill, verb, nbundles,
  ...
  if(length(K)==1 && bf==FALSE){
    ...
    fit <-  smash.tpxfit(X=X, theta=initheta, alpha=alpha, tol=tol, verb=verb,
                   ...
                   top_genes=top_genes,
                   burn_in=burn_in)
    fit$D <- smash.tpxResids(X=X, theta=fit$theta, omega=fit$omega, grp=grp,
                       nonzero=nonzero)$D
    return(fit)
  }
  ...
  for(i in 1:nK){
    ...
    fit <- tpxfit(X=X, theta=initheta, alpha=alpha, tol=tol, verb=verb,
                  ...
                  grp=grp, tmax=tmax, wtol=wtol,
                  qn=qn, nbundles=nbundles, use_squarem, light=light, top_genes=top_genes,
                  burn_in = burn_in)

    BF <- c(BF, smash.tpxML(X=X, theta=fit$theta, omega=fit$omega, alpha=fit$alpha, L=fit$L, dcut=dcut, admix=admix, grp=grp) - null)
    R <- smash.tpxResids(X=X, theta=fit$theta, omega=fit$omega, grp=grp, nonzero=nonzero)
   ...
   #   return(bestfit)
          ...
          if(i<nK){
              if(!admix){ initheta <- smash.tpxinit(X,2,K[i+1], alpha, 0) }
              else{ initheta <- smash.tpxThetaStart(X, fit$theta, fit$omega, K[i+1]) }
          }
                   ...
                   use_squarem, light, top_genes, burn_in)
{
  ...
  Q0 <- col_sums(X)/sum(X)
  L <- smash.tpxlpost(X=X, theta=theta, omega=omega, alpha=alpha, admix=admix, grp=grp)
  ...
  while( update  && iter < tmax ){
    ...
    if(admix && wtol > 0 && (iter-1)%%nbundles==0){
      if(iter <= burn_in){
        Wfit <- smash.tpxweights(n=nrow(X), p=ncol(X), xvo=xvo, wrd=wrd, doc=doc,
      ...
      if(iter > burn_in){
       # system.time(suppressWarnings(select_genes <- as.numeric(na.omit(as.vector(ExtractTopFeatures(theta, top_genes))))))
        ...
        if(length(which(rowSums(counts.mod)==0))==0){
          Wfit <- smash.tpxweights(n=nrow(Xmod), p=ncol(Xmod), xvo=xvo2, wrd=wrd2, doc=doc2,
          ...
          Wfit[which(rowSums(counts.mod)!=0),] <- smash.tpxweights(n=nrow(Xmod), p=ncol(Xmod), xvo=xvo2, wrd=wrd2, doc=doc2,
                                                             start=omega, theta=theta[select_genes,],  verb=0, nef=TRUE, wtol=wtol, tmax=20)
        }
...
#     if(admix && wtol > 0 && (iter-1)%%nbundles==0)
     ##if(admix && wtol > 0)
#      { Wfit <- smash.tpxweights(n=nrow(Xmod), p=ncol(X), xvo=xvo, wrd=wrd, doc=doc,
#        start=omega, theta=theta,  verb=0, nef=TRUE, wtol=wtol, tmax=20) }else{ Wfit <- omega }


#    move2 <- smash.tpxEM(X=X, m=m, theta=theta, omega=Wfit, alpha=alpha, admix=admix,
#                   method_admix=1, grp=grp)
    ...
    Wfit <- smash.normalizetpx(Wfit + 1e-15, byrow=TRUE);
  ...
  #  param_vec_in <- c(as.vector(omega),as.vector(theta));
    res <- squarem(par=as.numeric(param_vec_in),
                   fixptfn=smash.tpxsquarEM,
                   objfn= smash.tpxlpost_squarem,
                   X=X,
                   m=m,
                   K=K,
                   alpha=alpha,
   ...
   #  res_theta <- matrix(res$par[-(1:(nrow(X)*K))], nrow=ncol(X), ncol=K);
    ...
    Wfit <- smash.normalizetpx(Wfit + 1e-15, byrow=TRUE);
    theta <- smash.normalizetpx(theta + 1e-15, byrow=FALSE);
    move <- smash.tpxEM(X=X, m=m, theta=theta, omega=Wfit, alpha=alpha, admix=admix,
  ...
  out <- list(theta=theta_smoothed, omega=omega, K=K, alpha=alpha, L=L, iter=iter)
  invisible(out) }
...
smash.tpxEM <- function(X, m, theta, omega, alpha, admix, method_admix, grp)
{
  n <- nrow(X)
  p <- ncol(X)
  K <- ncol(theta)
  ...
  if(admix==TRUE & method_admix==1){
    ...
    theta <- smash.normalizetpx(t_matrix+alpha, byrow=FALSE)
    omega <- smash.normalizetpx(w_matrix+(1/(n*K)), byrow=TRUE)
  ...
  if(admix==TRUE & method_admix==2){ Xhat <- (X$v/smash.tpxQ(theta=theta, omega=omega, doc=X$i, wrd=X$j))*(omega[X$i,]*theta[X$j,])
             Zhat <- .C("Rzhat", n=as.integer(n), p=as.integer(p), K=as.integer(K), N=as.integer(nrow(Xhat)),
                        ...
                        zj = as.double(rep(0,K*p)), zi = as.double(rep(0,K*n)), PACKAGE="smashtpx")
             theta <- smash.normalizetpx(matrix(Zhat$zj+alpha, ncol=K), byrow=FALSE)
             omega <- smash.normalizetpx(matrix(Zhat$zi+1/K, ncol=K)) }
  if(!admix){
    qhat <- smash.tpxMixQ(X, omega, theta, grp, qhat=TRUE)$qhat
    ## EM update
    theta <- smash.normalizetpx(tcrossprod_simple_triplet_matrix( t(X), t(qhat) ) + alpha, byrow=FALSE)
    omega <- smash.normalizetpx(matrix(apply(qhat*m,2, function(x) tapply(x,grp,sum)), ncol=K)+1/K )  }
